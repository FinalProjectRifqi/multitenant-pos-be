...
const COMPLETED_STATUS_SQL = `
  UPPER(REPLACE(COALESCE(os.order_status_code, os.order_status_name), ' ', '_')) = 'SELESAI'
`;

const CANCELLED_STATUS_SQL = `
  UPPER(REPLACE(COALESCE(os.order_status_code, os.order_status_name), ' ', '_')) = 'DIBATALKAN'
`;
...
export class AnalyticsRepository implements IAnalyticsRepository {
  ...
  async getStatusTransactions(
    ...
    const rows = await this.applyOrderScope(
      ...
      .select([
        this.db.raw(
          "COALESCE(os.order_status_code, os.order_status_name, 'UNKNOWN') as status_code",
        ),
        this.db.raw("COALESCE(os.order_status_name, 'Unknown') as status_name"),
        this.db.raw('COUNT(o.order_id) as total_transactions'),
      ])
      .groupBy(['os.order_status_code', 'os.order_status_name'])
  ...
  private applyDateScope(
    ...
    if (scope.startDate) {
      query.whereRaw(`DATE(${column}) >= ?`, [scope.startDate]);
    }
    if (scope.endDate) {
      query.whereRaw(`DATE(${column}) <= ?`, [scope.endDate]);
    }
  }
